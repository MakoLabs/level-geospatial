...
module.exports = function(db, opts){
	...
	function put(position, id, value, callback){
		var newKey = encodeLevelKey(position, id);
  	        var hashKey = 'STORE_BY_PRECISION' in process.env ? latLonToQuadKey(position, hashPrecision) : "fixed";
	...
	function get(position, id, callback){
		...
		var options = { 'hash': 'STORE_BY_PRECISION' in process.env ? latLonToQuadKey(position, hashPrecision) : "fixed" };
	...
	function del(id, callback){
		idToKey(id, function(err, oldKey){
			...
			if (oldKey){
				...
				var hash = 'STORE_BY_PRECISION' in process.env ? oldKey.substring(0, hashPrecision) : "fixed";
				var batch = [];
	...
	function search(position, radius, options){
		...
		var stream = new Readable({objectMode : true});
 	    var hash = 'STORE_BY_PRECISION' in process.env ? latLonToQuadKey(position, hashPrecision) : "fixed";

...
module.exports = function(db, opts){
	...
	function put(position, id, value, callback){
		var newKey = encodeLevelKey(position, id);
  	        var hashKey = latLonToQuadKey(position, 10);
	...
	function get(position, id, callback){
		...
		var options = { 'hash': latLonToQuadKey(position, 10) };
	...
	function del(id, callback){
		idToKey(id, function(err, oldKey){
			...
			if (oldKey){
				...
				//var hash = latLonToQuadKey(decoded.position, 10);
				var hash = oldKey.substring(0, 10);
				var batch = [];
	...
	function search(position, radius){
		var stream = new Readable({objectMode : true});
 	    var hash = latLonToQuadKey(position, 10);

...
module.exports = function(db, opts){
	...
	var storeByPrecision = opts && 'storeByPrecision' in opts && opts.storeByPrecision;
		...
		quadKeys.forEach(function(quadKey){
			...
			db.createReadStream(options)
			  .on('data', function (data) {
				...
				var distanceFunc = cu.distanceEquiRect;
				if (process.env.DISTANCE_FUNC && process.env.DISTANCE_FUNC == 'SPHERE'){
					distanceFunc = cu.distanceSphereCos;
				} else if (process.env.DISTANCE_FUNC && process.env.DISTANCE_FUNC == 'HAVERSINE'){
					distanceFunc = cu.distance;
				}
			  	
			  	var d = distanceFunc(parseFloat(position.lat),parseFloat(position.lon),key.position.lat,key.position.lon);
